...
function HuntDashboard() {
  ...
  const onMutationSuccess = (data: CollectionEntry) => {
    queryClient.setQueryData(queryKeys.collectionEntry(data.pokemon_id, data.form_id), data);
    invalidateCollectionAggregates(queryClient);
  };

  const counterMutation = useMutation({
    mutationFn: ({ pokemonId, formId, amount }: { pokemonId: number; formId: number; amount: 1 | 10 | 100 }) =>
  ...
  const resetMutation = useMutation({
    mutationFn: ({ pokemonId, formId }: { pokemonId: number; formId: number }) => resetHunt(pokemonId, formId),
    onSuccess: onMutationSuccess,
  });
  ...
  return (
    <div className="flex flex-col h-full w-full">
      <div className="h-14 flex items-center px-6 border-b border-border">
        ...
        <span className="ml-auto text-xs text-muted-foreground">{hunts?.length ?? 0} active</span>
      </div>
      <div className="flex-1 overflow-auto p-6">
        {isLoading && <p className="text-sm text-muted-foreground">Loading…</p>}
        {!isLoading && (hunts?.length ?? 0) === 0 && (
          <p className="text-sm text-muted-foreground">
            No active hunts — mark a Pokémon as "Hunting" from its detail page to start tracking it
            here.
          </p>
        )}
              ...
              <Card key={entry.id}>
                <CardContent className="space-y-3">
                  <div className="flex items-center gap-3">
                    {p?.sprite_url && <img src={p.sprite_url} alt="" className="size-10" />}
                    <div className="min-w-0 flex-1">
                      ...
                      {best && (
                        <p className="text-xs text-muted-foreground">
                          Best: {GAME_LABELS[best.game]} · {METHOD_LABELS[best.method]} ·{" "}
                          {formatOdds(entry.has_shiny_charm ? best.odds_charm : best.odds_base)}
                        </p>
                      )}
                    </div>
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => resetMutation.mutate({ pokemonId: entry.pokemon_id, formId: entry.form_id })}
                    >
                    ...
                    <div className="ml-auto flex items-center gap-1.5">
                      {([1, 10, 100] as const).map((amount) => (
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            counterMutation.mutate({ pokemonId: entry.pokemon_id, formId: entry.form_id, amount })
                          }
                        >
                          +{amount}
                        </Button>
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    {(
                      ...
                      <label key={field} className="flex items-center gap-1.5 text-sm text-muted-foreground">
                        <Switch
                          size="sm"
                          checked={entry[field]}
                    ...
                    {entry.chain_count > 0 && <Badge variant="outline">Chain ×{entry.chain_count}</Badge>}
                  </div>
                </CardContent>
              </Card>
            );
