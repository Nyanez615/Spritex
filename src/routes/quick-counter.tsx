...
function QuickCounter() {
  ...
  if (!hunts || hunts.length === 0) {
    return <CenteredMessage text="No active hunts yet — mark a Pokémon as “Hunting” from its detail page." />;
  }

  const entry = hunts[index];
  ...
  return (
    <div className="flex flex-col h-full w-full">
      <div className="h-14 flex items-center justify-center px-6 border-b border-border">
        <h1 className="text-base font-semibold text-foreground">Quick Counter</h1>
      </div>
      <div className="flex-1 flex flex-col items-center justify-center gap-6 p-6">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="icon-lg"
            disabled={hunts.length < 2}
            onClick={() => setIndex((i) => (i - 1 + hunts.length) % hunts.length)}
          >
            <ChevronLeft className="size-6" />
          </Button>

          <div className="flex flex-col items-center gap-2 w-56">
            {p?.sprite_url && <img src={p.sprite_url} alt="" className="h-28 w-28" />}
            <span className="text-lg font-semibold text-foreground">{p?.display_name ?? `#${entry.pokemon_id}`}</span>
            <span className="text-5xl font-bold text-foreground tabular-nums">{entry.encounter_count}</span>
            <span className="text-xs text-muted-foreground">
              {index + 1} of {hunts.length}
            </span>
          </div>
          ...
          <Button
            ...
            size="icon-lg"
            disabled={hunts.length < 2}
            onClick={() => setIndex((i) => (i + 1) % hunts.length)}
          >
        ...
        <div className="grid grid-cols-3 gap-3 w-full max-w-sm">
          {([1, 10, 100] as const).map((amount) => (
            <Button
              ...
              onClick={() => counterMutation.mutate({ pokemonId: entry.pokemon_id, formId: entry.form_id, amount })}
            >
              +{amount}
            </Button>
          ))}
